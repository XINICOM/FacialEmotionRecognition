...
from config import (DEVICE, NUM_EPOCHS, LEARNING_RATE, EARLY_STOP_PATIENCE,
                    MODEL_SAVE_PATH, EMOTION_LABELS)
import control
...
def train(model, train_loader, val_loader, start_epoch=0, best_val_acc=0.0,
    ...
    print(f"[train] 开始训练 | 设备: {DEVICE} | 总轮数: {NUM_EPOCHS}")
    ...
    for epoch in range(start_epoch, NUM_EPOCHS):
        ...
        print(f"[train] Epoch {epoch+1}/{NUM_EPOCHS} | "
              f"训练损失: {avg_train_loss:.4f} 训练精度: {train_acc:.4f} | "
              f"验证损失: {val_loss:.4f} 验证精度: {val_acc:.4f}")
        ...
        if val_acc > best_val_acc:
            ...
            if patience_counter >= EARLY_STOP_PATIENCE:
                print(f"[train] 早停触发 (连续 {EARLY_STOP_PATIENCE} 轮验证精度未提升)")
                break
...
def test(model, test_loader):
    ...
    for cls_id in range(len(EMOTION_LABELS)):
        ...
        if cls_total > 0:
            print(f"  {EMOTION_LABELS[cls_id]}: {cls_correct}/{cls_total} = {cls_correct/cls_total:.4f}")
